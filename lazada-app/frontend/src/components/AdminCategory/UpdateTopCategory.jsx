...
function UpdateSubCategory() {
  ...
  function addField() {
    ...
    if (input === "Name" || input === "name") {
      window.alert('Cant have an extra name column');
      return;
    }
    let item = {
      name: input.toLowerCase(),
            ...
            entry.map(entry => {
              ...
              if (key === "name") {
                ...
                return (
                  ...
                  <div key={key} className="border border-2 border-primary mt-2 d-flex justify-content-between">
                    <h1>{key}</h1>
                    <button className="btn btn-primary" onClick={() => renderUpdateForm(key)}>Update</button>
                  </div>
                )
          ...
          <fieldset>
            ...
            </div>
            {
              category.map((item, index) => {
                return (
                  <div key={index} className="container mt-5 border border-primary border-5">
                    <div><h4><strong>Field: </strong>{item.name}</h4></div>
                    <div><h4><strong>Type: </strong>{item.type}</h4></div>
                    <div><h4><strong>Require: </strong>{item.required}</h4></div>
                  </div>
