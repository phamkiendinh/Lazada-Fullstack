...
const CreateTopCategory = () => {
  ...
  async function saveTopCategory(event) {
    ...
    nameInput.categoryName = nameInput.categoryName
      .toLowerCase()
      .replace(" ", "")
      .replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, "");
    let data = [
    ...
    console.log(data);
    try {
      const response = await fetch("http://localhost:3001/admin/category/create", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        <form onSubmit={saveTopCategory}>
          <fieldset>
            ...
            {category.map((item, index) => (
              <div
                key={index}
                className="container mt-5 border border-primary border-5"
              >
                <div>
                  <h4>
                    <strong>Field: </strong>
        ...
        <div id="addFieldForm" style={{ display: fieldFormVisibility }}>
          ...
          <div className="mb-3">
            ...
            <input
              ...
              name="required"
              value="true"
              checked={require === "true"}
              onChange={(e) => updateRequireInput(e)}
            />
            <label htmlFor="required">Required</label>
            <br />
            <input
